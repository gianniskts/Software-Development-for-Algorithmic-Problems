...
// 
// Provided kindly by colleague Tomazos Pavlos on lecture's e-class
...
protected:
  ...
  {
    if (!get(ipm, eh->first)){
      if (!get(ipm, eh->first->neighbor(eh->second)))
        return;
    } 

    CGAL::IO::Color c;
    c = t2.is_constrained(*eh) ? CGAL::IO::green() : CGAL::IO::black(); // Default coloring logic
